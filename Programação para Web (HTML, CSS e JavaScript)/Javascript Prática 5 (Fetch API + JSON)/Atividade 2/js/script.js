...
function mostrarApenasConta() {
    ...
    document.getElementById('statuscep').style.display = 'none'
    document.getElementById('statuslogradouro').style.display = 'none'
    document.getElementById('statusbairro').style.display = 'none'
    document.getElementById('statuscidade').style.display = 'none'
    document.getElementById('statusestado').style.display = 'none'
...
async function checaCEP() {
    if (document.getElementById('cep').value.length == 8) {
        ...
        if (!endereco.erro) {
            document.getElementById('statuscep').innerText = '✅'
    ...
    document.getElementById('statuscep').innerText = 'Não validado'
    return false
}
...
function validaCriacao(status_atual) {
    ...
    let cep = checaCEP()
    let logradouro = checaTexto("logradouro", "statuslogradouro")
    let bairro = checaTexto("bairro", "statusbairro")
    let cidade = checaTexto("cidade", "statuscidade")
    let estado = checaTexto("estado", "statusestado")
    if (nome && sobrenome && cpf && email && senha && senhaa && cep && logradouro && bairro && cidade && estado) {
